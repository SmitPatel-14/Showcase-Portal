      ...
      files?.screenshots?.length
        ? Promise.all(
            files.screenshots.map((f) =>
              upload(f, "projects/screenshots", "image"),
            ),
          )
        : Promise.resolve([]),
      files?.ppt?.[0]
        ? upload(files.ppt[0], "projects/ppts", "raw")
        : Promise.resolve(null),
      files?.pdf?.[0]
        ? upload(files.pdf[0], "projects/pdfs", "raw")
        : Promise.resolve(null),
      files?.video?.[0]
        ? upload(files.video[0], "projects/videos", "video")
        : Promise.resolve(null),
      files?.teamPhoto?.[0]
        ? upload(files.teamPhoto[0], "projects/team", "image")
        : Promise.resolve(null),
    ]);
...
export const getProjects = async (req, res, next) => {
  try {
    ...
    // search on title, techStack, category name
    if (search?.trim()) {
